...
export default function KurlarPage() {
  ...
  const reload = useCallback(() => setData(getKurlar()), []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addKur({ tarih: form.tarih, usdTl: parseFloat(form.usdTl) });
    setForm({ tarih: '', usdTl: '' });
    setShowModal(false);
    reload();
  };
    ...
    <div className="mx-auto max-w-3xl p-6">
              ...
              return (
                <tr key={k.id} className="border-b border-zinc-100 transition hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900/50">
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    {i > 0 && (
                      <span className={`ml-2 text-xs ${change > 0 ? 'text-red-500' : change < 0 ? 'text-green-500' : 'text-zinc-400'}`}>
                        {change > 0 ? '+' : ''}{change.toFixed(2)}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => handleDelete(k.id)} className="rounded px-2 py-1 text-xs text-red-500 transition hover:bg-red-50 dark:hover:bg-red-950">
                      Sil
                    </button>
                  </td>
                </tr>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Yeni Kur Ekle">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium">USD/TL Kuru</label>
            <input type="number" step="0.01" required value={form.usdTl} onChange={e => setForm({ ...form, usdTl: e.target.value })}
              placeholder="43.50" className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800" />
          </div>
          <button type="submit" className="rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-violet-700">
            Ekle
          </button>
        </form>
      </Modal>
